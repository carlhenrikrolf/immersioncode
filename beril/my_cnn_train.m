function [net, stats] = fake_cnn_train(net, imdb, getBatch, varargin)
...
vl_simplenn_display(net, 'batchSize', opts.batchSize) ; %% the batch size is needed to estimate memory usage

evaluateMode = isempty(opts.train) ;
if ~evaluateMode %% if some list of numbers were written in 'train'
  for i=1:numel(net.layers) %% for each layer
    J = numel(net.layers{i}.weights) ; %% number of weights
    if ~isfield(net.layers{i}, 'learningRate')
      net.layers{i}.learningRate = ones(1, J) ; %% sets the learning rate for each weight (?) to 1 in the layer
    end
...
if isstr(opts.errorFunction)
  switch opts.errorFunction
    case 'none'
      opts.errorFunction = @error_none ; %% What does "@"?mean? It's just a normal fn handle
      hasError = false ;
    case 'multiclass'
...
for epoch=start+1:opts.numEpochs %% remaining epochs (?)
  ...
  if params.plotStatistics %% just some plotting I guess
    switchFigure(1) ; clf ;
    plots = setdiff(...
      cat(2,...
...
%         batchStart = t + (labindex-1) + params.batchSize ; %%% "labindex" returns a 1 for me, but what is it?
  ...
  for j=numel(res(l).dzdw):-1:1
    ...
    if j == 3 && strcmp(net.layers{l}.type, 'bnorm')
      ...
      net.layers{l}.weights{j} = vl_taccum(...
        ...
        thisLR / batchSize, ...
        parDer) ; %%% How do you get from 'alpha', 'A' to 'alpha A'?
    else %% all of this is actually important!
      % Standard gradient training.
      thisDecay = params.weightDecay * net.layers{l}.weightDecay(j) ; %% important!
